    ...
    export default {
            ...
            '$store.state.Login.RefreshToken': async function(newVal, oldVal) {
                if ( oldVal != newVal ) {
                    ...
                    if ( newVal === '' || newVal === null ) {
                        ...
                        setTimeout(() => this.$store.commit('setRefreshToken', null), 1500)
                    } else {
            ...
            '$store.state.Login.TokenData': async function(newVal, oldVal) {
                if ( oldVal != newVal ) {
                    ...
                    if ( newVal === '' || newVal === null ) {
                        this.getRefreshToken()
                        this.detectCookiesStatusData()
                        setTimeout(() => this.$store.commit('setTokenAccess', null), 1500)
                    } else {
        ...
        methods: {
            ...
            async deleteLoginData() {
                this.$store.state.Tools.deleteCookies('kompas._token'); this.$store.state.Tools.deleteCookies('_km_dtl_d'); this.$store.state.Tools.deleteCookies('_km_dtl_s')
            },
