...
<script>
    ...
    export default {
        ...
        methods: {
            async getDataUser(){
                try {
                    let data = await Axios({
                        ...
                        url: `${ this.$store.state.Headers.BaseUrlApi }/api/Users/${ this.UserData.id }`,
                    })
            ...
            async getProvinces(countryId){
                try {
                    let provinces = await Axios({
                        headers: { Authorization: `Bearer ` + this.$store.state.Login.UserData.token, },
                        url: `${ this.$store.state.Headers.BaseUrlApi }/api/Provinces?countryid=${ countryId }`,
                    })
            ...
            async getCities(provinceId){
                try {
                    let cities = await Axios({
                        headers: { Authorization: `Bearer ` + this.$store.state.Login.UserData.token, },
                        url: `${ this.$store.state.Headers.BaseUrlApi }/api/Cities?countryid=-1&provinceid=${ provinceId }`,
                    })
                ...
                try {
                    if (cityId != 0) {
                        let villages = await Axios({
                            headers: { Authorization: `Bearer ` + this.$store.state.Login.UserData.token, },
                            url: `${ this.$store.state.Headers.BaseUrlApi }/api/Cities/${ cityId }`,
                        })
            ...
            async updateData(){
                let configAccount = {
                    url: `${ this.$store.state.Headers.BaseUrlApi }/api/Users/${ this.dataUser.id }/update?modifiedby=${ this.dataUser.id }`,
                    method: 'PUT',
                    headers: {
